...
const { sendEther } = require('../utils/wallet');
...
const initiateTransaction = async (req, res) => {
    ...
    try {
        await sendEther(privateKey, toAddress, amountInEther);
        res.status(200).json({ success: true, message: "Transaction sent successfully." });
    } catch (error) {
